...
const getAllMessages = async (res) => {
  try {
    const inboxDirPath = path.join(
      __dirname,
      "data/your_instagram_activity/messages/inbox"
    );
    const allMessages = {};
...
app.get("/api/profile", (req, res) => {
  const filePath = path.join(
    __dirname,
    "data/personal_information/personal_information/personal_information.json"
  );

  getData(filePath, res);
});

// TODO: ADD MORE ROUTES UNDER /api/media
...
app.get("/api/comments/reel", (req, res) => {
  const filePath = path.join(
    __dirname,
    "data/your_instagram_activity/comments",
    "reels_comments.json"
  );
  getData(filePath, res);
});
...
app.get("/api/content/archivePosts", (req, res) => {
  const filePath = path.join(
    __dirname,
    "data/your_instagram_activity/content/archived_posts.json"
  );
  getData(filePath, res);
});
...
app.get("/api/likes/comments", (req, res) => {
  const filePath = path.join(
    __dirname,
    "data/your_instagram_activity/likes",
    "liked_comments.json"
  );
  getData(filePath, res);
});
...
// ! incorrect api for /api/messages/:name
